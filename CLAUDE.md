...
```
mabda/
├── src/                29 GPU library modules (@public + @internal markers on line 1)
...
├── dist/               Bundled mabda.cyr (regenerated by scripts/bundle.sh)
...
├── cyrius.toml         Cyrius build + [lib] manifest + deps
├── Makefile            test-all, bench, version-check, bundle
└── VERSION
```

Tests are invoked from repo root: `make test-all` (runs version-check and
bundle regen before the test pass).

The frozen Rust v1.0.0 source is available via `git checkout 1.0.0`.
Reference benchmark numbers at `docs/rust-v1-bench-history.csv`.
